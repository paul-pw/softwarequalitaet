...
Die Formel für die Berechnung des CER ist in der Formel~\ref{eq:cer} angegeben.
...
% Ohne die weaknesses beschreiben, einfach sagen man geht auf die betroffenen CVEs ein und nimmt da den durchschnitt.
...
schlagen die Forscher vor die Anzahl der CVEs einer Schwachstelle (K) mit dem Zeitraum (in Monaten), in dem die Schwachstelle in dem System existierte (M) zu verrechnen (siehe Gleichung~\ref{eq:cve_risk}).
\begin{equation} \label{eq:cve_risk}
  R = \frac{ K }{ M }
\end{equation}
Abschließend wird die durchschnittliche CVSS Bewertung aller Schwachstellen mit dem Missbrauchsrisiko verrechnet (siehe Gleichung~\ref{eq:cve_risk2}).
...
In Tabelle X und Y ist aufgeschlüsselt welche Charakteristika der ISO 25010 durch die Metriken abgedeckt werden.
Da in der Analysephase die Grundlagen für alle weiteren Phasen der Softwareentwicklung gelegt werden, 
deckt der \alpha-Wert alle Charakteristika der ISO 25010 ab.
Tabelle X gibt einen Überblick über die Abdeckung der Metriken des ersten Artikels~\cite{Jain2014SecurityMA}
...
Tabelle Y gibt einen Überblick über die Abdeckung der Metriken der anderen beiden Paper~\cite{Chowdhury_Chan_Zulkernine_2008}~\cite{Wang_Wang_Guo_Xia_2009}.
...
Da die Art der Schwachstellen vielseitig ist können so abhängig von den betrachteten Systemen und Komponenten auch alle Charakteristika der ISO 25010 abgedeckt werden.

Zusammengenommen bieten die Metriken der Paper eine gute Abdeckung der Charakteristika der ISO 25010 und 
